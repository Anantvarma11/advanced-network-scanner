...
def update_topology(n, selected_devices, selected_services):
    ...
    for ip in df['IP'].unique(): G.add_node(ip, label=df[df['IP']==ip]['Hostname'].iloc[0], device=df[df['IP']==ip]['Device'].iloc[0])
    for _, row in df.iterrows(): G.add_edge(row['IP'], row['Port'], service=row['Service'])
    ...
    for service,color in service_colors.items():
        ...
        for u,v,d in G.edges(data=True):
            if d.get('service')==service:
                x0,y0=pos[u]; x1,y1=pos[u]; x_edges.extend([x0,x1,None]); y_edges.extend([y0,y1,None])
    ...
    for node in G.nodes():
        ...
        host_df = df[df['IP']==node]; num_ports=len(host_df)
        os_type = host_df['OS'].iloc[0] if not host_df.empty else "Unknown"
        node_text.append(f"{host_df['Hostname'].iloc[0]} ({node}) - {G.nodes[node]['device']} - {os_type} - {num_ports} ports")
...
def show_host_details(clickData):
    ...
    clicked_ip = clickData["points"][0]["text"]
    with data_lock: df=pd.DataFrame(open_ports_data)
...
Docker Deploymentrequirements.txt : 
...
COPY scanner.py .
EXPOSE 8050
CMD ["python","scanner.py"]Build & rundocker build -t prod-network-scanner .
