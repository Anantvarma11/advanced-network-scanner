...
scanning = True  # Flag for scan cancellation
...
def detect_device(ip, ports, os_type):
    ...
    if 22 in ports_set and os_type == "Linux/Unix":
        return "Linux PC"
    if 23 in ports_set:
        return "IoT Device"
    if 515 in ports_set or 631 in ports_set:
        return "Printer"
...
def scan_port(ip, port, ip_open_ports):
    try:
        ...
        if resp is not None and resp.haslayer(TCP) and resp[TCP].flags == 0x12:
            ...
            ip_open_ports[ip].append((port, service, os_type))
    except Scapy_Exception:
        pass  # Ignore packet errors silently

# ----------------------------
# SCANNING LOOP
# ----------------------------
def start_scan_loop():
    global open_ports_data, live_hosts, scanning
    ...
    while scanning:
        ...
        if not live_hosts:
            ...
            ip_open_ports = defaultdict(list)
            tasks = []
                ...
                for i in range(0, len(live_hosts), batch_size):
                    ...
                    for ip in batch:
                        for port in ports_to_scan:
                            tasks.append(executor.submit(scan_port, ip, port, ip_open_ports))
                for future in tqdm(tasks, desc="Port Scanning"):
                    future.result()
            # Process collected ports and append to global data
            for ip in live_hosts:
                open_list = ip_open_ports[ip]
                if open_list:
                    hostname = resolve_hostname(ip)
                    os_type = open_list[0][2] if open_list else "Unknown"
                    all_ports = [p for p, _, _ in open_list]
                    device_type = detect_device(ip, all_ports, os_type)
                    for p, s, o in open_list:
                        with data_lock:
                            open_ports_data.append({
                                "IP": ip, "Hostname": hostname, "Port": p,
                                "Service": s, "OS": os_type, "Device": device_type
                            })
            # Auto-save report
            with data_lock:
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.Label("Select Device Types:"),
            dcc.Dropdown(
                id='device-filter',
...
def update_topology(n, selected_devices, selected_services):
    ...
    edge_traces = []
    service_colors = {
        "HTTP": "blue", "HTTPS": "darkblue", "SSH": "green", "RDP": "red",
        "FTP": "orange", "Telnet": "purple", "MySQL": "brown", "VNC": "pink", "Other": "grey"
    }
    for service, color in service_colors.items():
        ...
        edge_traces.append(go.Scatter(
            x=x_edges, y=y_edges, line=dict(width=2, color=color),
            hoverinfo='text', mode='lines', name=service
        ))
    node_x, node_y, node_text, node_color, node_custom_ip = [], [], [], [], []
    color_map = {
        "Router/Server": "red", "Windows PC": "blue", "Linux PC": "green",
        "IoT Device": "orange", "Printer": "purple", "Unknown Device": "grey"
    }
    for node in G.nodes():
        x, y = pos[node]
        node_x.append(x)
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title='Ultimate Network Topology',
    ...
    metrics_text = [
        html.P(f"Total Live Hosts: {total_hosts}"),
...
def show_host_details(clickData):
    ...
    details = html.Div([
        html.H4(f"{hostname} - {device_type}"),
        html.P(f"OS: {os_type}"),
        html.Ul(port_info)
    ])
    ...
    if not critical.empty:
        alerts = html.Div([
            html.H4("⚠ Critical Ports Detected!"),
...
def export_csv(n_clicks):
    with data_lock:
        df = pd.DataFrame(open_ports_data)
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    return dcc.send_data_frame(df, filename=f"network_report_{timestamp}.csv", index=False)
